...
BOOLEAN test_free_node_debug_poison_before_free()
{
    ...
    TEST_ASSERT(g_last_freed_node_type_before_free == (UCHAR)0xFF,
        "In DEBUG, node->type must be poisoned (0xFF) before free");
...
BOOLEAN test_free_node_various_types()
{
    ...
    for (ULONG i = 0; i < RTL_NUMBER_OF(kTypes); ++i) {
        ...
        TEST_ASSERT(g_last_freed_node_type_before_free == (UCHAR)0xFF,
            "DEBUG: observed poisoned type before free");
...
BOOLEAN test_free_node_bulk_reverse_order()
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        addrs[i] = &nodes[i];            // always set address
        if (nodes[i]) ++alloc_count;     // count only successful allocs
    }

    for (int i = N - 1; i >= 0; --i) {
        free_node(addrs[i]);             // safe: handles *ptr == NULL
    }

    TEST_ASSERT(g_free_call_count == (ULONG)alloc_count, "Must free exactly allocated nodes");
...
BOOLEAN test_free_node_logging_guidance()
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (n) {
            DbgPrint("[TEST] About to free node #%d at %p (type=%d) — expect LOG_MSG before free\n",
                i, n, n->type);
            ART_NODE** pn = &n;
            free_node(pn);
            DbgPrint("[TEST] free_node returned for node #%d\n", i);
        }
    }
...
// ========== Test 9: Safety — double-indirection only nulls the passed variable ==========
BOOLEAN test_free_node_pointer_isolation()
{
    ...
    // DO NOT call free_node(&alias); that would be a double-free — we assert semantics only
    TEST_ASSERT(alias != NULL, "Alias variable retains stale value (as designed)");
    DbgPrint("[INFO] Alias pointer intentionally left dangling to validate API semantics\n");
...
BOOLEAN test_free_node_edge_allocations()
{
    ...
    // Exactly sizeof(ART_NODE) — already used throughout; add a pattern touch before free
    ART_NODE* n = (ART_NODE*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(ART_NODE), ART_TAG);
    if (n) {
        ...
        TEST_ASSERT(g_free_call_count == 1, "Freed once");
        TEST_ASSERT(g_last_freed_tag == ART_TAG, "Used ART_TAG");

        TEST_ASSERT(g_last_freed_node_type_before_free == (UCHAR)0xFF,
            "DEBUG: poisoned type before free");


    }
    else {
        DbgPrint("[TEST SKIP] Could not allocate ART_NODE for edge test\n");
    }
...
{
    DbgPrint("\n========================================\n");
    DbgPrint("Starting Comprehensive free_node Test Suite\n");
    DbgPrint("========================================\n\n");
    ...
    DbgPrint("\n========================================\n");
    if (all_passed) {
        DbgPrint("ALL free_node TESTS PASSED! \n");
    }
    else {
        DbgPrint("SOME free_node TESTS FAILED! \n");
    }
    DbgPrint("========================================\n\n");
