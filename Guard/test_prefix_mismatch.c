...
//    n4->base.type = NODE4; n4->base.num_of_child = 1;
//
//    // prefix: 0x40,41,42,43 (stored header)
//    t_set_node_prefix(&n4->base, 4, 0x40);
//
//    // leaf ve key birbirinin aynısı olsun: 0x40,41,42,43,...
//    ART_LEAF* lf = test_alloc_leaf(8, 0x40); TEST_ASSERT(lf, "pre: leaf alloc");
//    n4->children[0] = SET_LEAF(lf);
//
//    // key temel olarak leaf gibi, ama header'la çelişen bir bayt üretelim:
//    // örn. stored header’daki i=2 baytını key’de 0x42 bırakalım (doğru),
//    // header’ı bozmak için node->base.prefix[2] = 0x99 yapalım.
//    n4->base.prefix[2] = 0x99; // drift
//
//    PUCHAR key = t_alloc_key(8, 0x40); TEST_ASSERT(key, "pre: key alloc");
//
//    USHORT r = prefix_mismatch(&n4->base, key, 8, 0);
//    // Drift toleransı sayesinde first_window (4) kadar ilerlemeli.
//    TEST_ASSERT(r == 4, "drift leaf tarafından doğrulandığında full window eşleşmeli");
//
...
//    n4->base.type = NODE4; n4->base.num_of_child = 1;
//
//    // Uzun prefix (extended path)
//    t_set_node_prefix(&n4->base, (USHORT)(MAX_PREFIX_LENGTH + 4), 0x50);
//
//    // Ağacın minimum’u için uzun bir leaf bağlayalım
...
//
//    // Aynı zamanda rep_leaf verelim ama depth’i aşamayacak kadar kısa olsun
//    ART_LEAF* rep_short = test_alloc_leaf(4, 0x50); TEST_ASSERT(rep_short, "pre: short rep");
//
//    // key leaf_long ile uyumlu
//    PUCHAR key = t_alloc_key((USHORT)(MAX_PREFIX_LENGTH + 12), 0x50); TEST_ASSERT(key, "pre: key");
//
//    // depth, rep_short’u geçsin
//    USHORT depth = 6;
//
//    USHORT r = prefix_mismatch(&n4->base, key, (USHORT)(MAX_PREFIX_LENGTH + 12), depth);
//    // Beklenti: stored window (min(MAX_PREFIX_LENGTH, prefix_length, remaining)) + extended tarafta da ilerleyebilsin.
...
//
//    TEST_ASSERT(r == (USHORT)(first_window + to_check), "rep_leaf kısa olsa da minimum() ile extended devam edebilmeli");
//
...
//    if (!test_prefix_mismatch_drift_tolerated_by_leaf())      all_passed = FALSE;
//    if (!test_prefix_mismatch_rep_leaf_too_short_falls_back())all_passed = FALSE;
//
//
//    LOG_MSG("\n========================================\n");
//    if (all_passed) {
